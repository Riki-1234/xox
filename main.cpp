...
void pc_choose(int choice, char* board){
    ...
    const int index1 = pc_odg - 1;
    int pc_odg2 = rand() % 9 + 1;
    const int index2 = pc_odg2 - 1;
    const char pc2 = choice == 1 ? 'O' : 'X';

    if (board[index1] != 'O' && board[index1] != 'X') {
        board[index1] = pc;
    }
    else if(board[index2] != 'O' && board[index2] != 'X') {
        board[index2] = pc2;
    }
}
